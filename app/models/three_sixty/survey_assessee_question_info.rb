# == Schema Information
#
# Table name: three_sixty_survey_assessee_question_infos
#
...
#  three_sixty_reviewer_group_id  :integer          default(0), not null
#

class ThreeSixty::SurveyAssesseeQuestionInfo < ActiveRecord::Base
  self.table_name = :three_sixty_survey_assessee_question_infos

  has_many :related_question_infos, :class_name => 'ThreeSixty::SurveyAssesseeQuestionInfo', :foreign_key => :three_sixty_question_id, :primary_key => :three_sixty_question_id
  ...
  validates :three_sixty_question_id, :presence => true, :uniqueness => { :scope => [:three_sixty_survey_assessee_id, :three_sixty_reviewer_group_id] }
end
