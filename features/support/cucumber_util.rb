...
class CucumberAjaxCallTracker
  def self.enabled?
    return ((ENV['CUCUMBER_ENV'] || ENV['TDDIUM']) && !ENV['CUCUMBER_DISABLE_PENDING_AJAX_CHECKS'])
  end
  ...
  def self.wait_till_ajax_calls_complete(page)
    begin
      ...
      pending_ajax_calls = page.evaluate_script('window.jQuery!= undefined && jQuery.active')

      while pending_ajax_calls && pending_ajax_calls > 0 && check_count > 0
        check_count -= 1
        sleep wait_interval
        pending_ajax_calls = page.evaluate_script('window.jQuery!= undefined && jQuery.active')
      end
