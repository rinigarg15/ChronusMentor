...
;(function($, window, undefined) {
    ...
    $.widget("uix.uixmultiselect", {
        ...
        _create: function() {
                ...
                .append(
                    $('<div></div>').addClass('multiselect-selected-list')
                        .append( $('<div></div>').addClass('ui-widget-header')
                            ...
                            .append( selListHeader = $('<div></div>').addClass('header-text').css('float', 'left') )
                        )
                ...
                ['right,top'.indexOf(this.options.availableListPosition)>=0?'prepend':'append'](
                    $('<div></div>').addClass('multiselect-available-list')
                        .append( $('<div></div>').addClass('ui-widget-header')
                            ...
                            .append( avListHeader = $('<div></div>').addClass('header-text').css('float', 'left') )
            ...
            UiWidgetLabels.addAndAssociateLabels("input.uix-search.ui-widget-content", "uix_multiselect_quick_search_", jQuery.ui.multiselect.locale.searchAmongOptions);
        },
                ...
                this._buttons['search'] = $('<button></button>', { type:"button" }).addClass('uix-control-right')
                    ...
                    .click(function(e) {
                        ...
                        if (that._searchField.is(':visible')) {
                            var b = $(this);
                            window.setTimeout(function(){that._headers[searchHeader].show()}, 250);
                            ...
                            that._headers[searchHeader].hide();
        ...
        _applyListDroppable: function() {
            ...
            if (this.options.sortable) {
                ...
                this._lists['selected'].sortable({
                     ...
                     items: '.multiselect-element-wrapper div', // chronus change : only items and not the group
        ...
        _setLocale: function(locale) {
            ...
            if (!$.uix.uixmultiselect.i18n[locale]) {
        ...
        _resize: function() {
            ...
            if (this._searchField) {
                this._searchField.width( (sSize === 'Width' ? cAv : this.element.width()) - (isToggle ? 52 : 26) - 20 );  // issue #50 and chronus customiztion
            }

            // calculate inner lists height
            this._lists['available'].height(hAv - this._headers['available'].parent().outerHeight() - 20);  // account for borders and chronus customiztion
            this._lists['selected'].height(hSl - this._headers['selected'].parent().outerHeight() - 20);    // account for borders and chronus customiztion
        },
    ...
    OptionCache.prototype = {
        ...
        _createElement: function(optElement, optGroup) {
            ...
            var e = $('<div></div>').append(o).addClass('ui-state-default option-element')
                ...
                .hover(
                    function() {
                        if (optElement.prop('selected')) $(this).removeClass('');
                        $(this).addClass('ui-state-hover');
                    },
                    function() {
                        $(this).removeClass('ui-state-hover');
                        if (optElement.prop('selected')) $(this).addClass('');
                    }
                )
                .prepend('<span class="ui-icon ui-icon-plus pull-right"></span>')
                .prepend('<span class="ui-icon ui-icon-minus pull-right"></span>');
     ...
     */
    function _(p) {
        var locale = $.uix.uixmultiselect.i18n[p.locale] ? p.locale : '';
        var i18n = $.uix.uixmultiselect.i18n[locale];
