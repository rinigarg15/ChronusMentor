...
$.widget("ui.multiselect", {
  ...
  _create: function() {
    ...
    this.availableActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><span class="count">'+$.ui.multiselect.locale.availableFieldsText+'</span><span class="busy">'+$.tmpl($.ui.multiselect.locale.busy)+'</span><input type="text" class="search ui-widget-content ui-corner-all"/><a href="#" class="add-all">'+$.tmpl($.ui.multiselect.locale.addAll)+'</a></div>').appendTo(this.availableContainer);
    ...
    // set dimensions
    this.container.width(this.element.width()+1 + 75);
    ...
    // The code below is needed as the selected fields count is not displayed correctly \ 
    // when the enclosing div of the select tag is hidden
    setTimeout(function(){
      var count = that.selectedList.children('li:not(.ui-helper-hidden-accessible,.ui-sortable-placeholder)').size();
      that.selectedContainer.find('span.count').text($.tmpl($.ui.multiselect.locale.itemsCount, {count:count}));
    }, 500);

    //Section 508 compliance changes
    UiWidgetLabels.addAndAssociateLabels("input.search.ui-widget-content", "ui_multiselect_quick_search_", jQuery.ui.multiselect.locale.searchAmongOptions);
  },
  ...
  _getOptionNode: function(option) {
    option = $(option);
    var node = $('<li class="draggable ui-state-default ui-element"><span class="ui-icon"/>'+option.text()+'<a href="#" class="ui-state-default action"><span class="ui-corner-all ui-icon"/></a></li>').hide();
  ...
  _moveOptionNode: function(item) {
    ...
    setTimeout( function() {
      ...
      if (optionLink) {
        // Removing the visible in the selector as it was messing with the order of the admin view columns,
        // This was hapenning only in admin_views#new i.e, when the select field is hidden (the plugin is fetching options which are visible only, and all the options are hidden).
        var prevItem = item.prev('li:not(.ui-helper-hidden-accessible,.ui-sortable-placeholder)');
...
$.extend($.ui.multiselect, {
  ...
  locale: {
    ...
    itemsCount:'Selected Fields - #{count} fields',
    ...
    availableFieldsText: "Available Fields",
    searchAmongOptions: "Search among options"
  },
  constants: {
    MESSAGE_WARNING: 0,
    MESSAGE_EXCEPTION: 1,
