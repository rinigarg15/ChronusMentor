# This file is for usage with rails log analyer. It prints the 
# memory statistics of actions along with the other rla stats.
#
# Usage: rails-log-analyzer --format rails_log_file_format_with_memory.rb <logfiles> 
#
...
module RequestLogAnalyzer::FileFormat
  ...
  class RailsLogFileFormatWithMemory < Base
    ...
    def self.report_trackers(lines)
      ...
      analyze.numeric_value :memory,
        :category => REQUEST_CATEGORIZER,
        :title => "Memory",
        :line_type => :memory
 
      # analyze.numeric_value :memory_diff,
      #   :category => REQUEST_CATEGORIZER,
      #   :title => "Memory Diff",
      #   :line_type => :memory_diff
 
      analyze.frequency :category => REQUEST_CATEGORIZER, :title => 'Memory takers (> 190MB)',
        :if => lambda { |request| request[:memory] && request[:memory] > 190 }
      
      # analyze.frequency :category => REQUEST_CATEGORIZER, :title => 'Memory diff (> 75MB)',
      #   :if => lambda { |request| request[:memory_diff] && request[:memory_diff] > 75 }
    ...
    LINE_DEFINITIONS = {
      ...
      :memory => RequestLogAnalyzer::LineDefinition.new(:memory,
          :teaser   => /Memory usage:/,
          :regexp   => /Memory usage: (\d+) \| PID: \d+/,
          :captures => [ { :name => :memory, :type  => :memory, :unit => :mb } ])
      
      # :memory_diff => RequestLogAnalyzer::LineDefinition.new(:memory_diff,
      #     :teaser   => /Memory usage:/,
      #    :regexp   => /Memory usage: (\d+) \| PID: \d+/,
      #     :captures => [ { :name => :memory_diff, :type  => :memory_diff, :unit => :mb } ])
    }

    # Definitions of common combinations of lines that can be parsed
    LINE_COLLECTIONS = {
      :minimal     => [:processing, :completed, :memory, :memory],
      :production  => [:processing, :completed, :failure, :memory], 
    ...
    class Request < RequestLogAnalyzer::Request
      ...
      def convert_memory(value, definition)
        value.to_i / 1024.0
      end

      # def convert_memory_diff(value, d)
      #   @@prev_val ||= 0
      #   mem = value.to_i / 1024
      #   delta = mem - @@prev_val
      #   @@prev_val = mem
