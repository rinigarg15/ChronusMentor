class EmployeeRolePopulator < PopulatorTask
  def patch(options = {})
    return unless @program.default_role_names.include?(RoleConstants::EMPLOYEE_NAME)
    ...
    return if @program.users.active.for_role(RoleConstants::EMPLOYEE_NAME).count >= count
