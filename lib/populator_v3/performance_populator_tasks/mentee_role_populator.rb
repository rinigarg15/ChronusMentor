class MenteeRolePopulator < PopulatorTask
  def patch(options = {})
    return unless @program.default_role_names.include?(RoleConstants::STUDENT_NAME)
    ...
    return if @program.users.active.students.count >= count
    add_roles(user_ids, count, @options)
  end

  def add_roles(user_ids, count, options = {})
    program = options[:program]
    role = program.roles.find_by(name: "RoleConstants::STUDENT_NAME".constantize)
