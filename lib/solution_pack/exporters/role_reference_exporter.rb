class RoleReferenceExporter < SolutionPack::Exporter
  ...
  def initialize(program, parent_exporter)
    if (parent_exporter.class == ForumExporter)
      self.objs = RoleReference.joins(:role).where(roles: { program_id: program.id }).select{|rr| rr.ref_obj_type == "Forum"}
    elsif (parent_exporter.class == SurveyExporter)
