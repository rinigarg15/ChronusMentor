...
#
# Usage:
# => DjSplit.new(queue_options: {queue: queue_name, max_workers: 4}, split_options: {size: 1000, by: 2}).enqueue(match_client, "bulk_mentor_match", student_ids, mentor_ids)
# instead of:
# => match_client.bulk_mentor_match(student_ids, mentor_ids)
#
# max_workers - Maximum number of workers that should be running in parallel for this job group.
#
...
#
# Other Split Options
# with_index - pass the index of the chunk that is processed to the function. eg. DjSplit.new(queue_options: {queue: "split"}, 
#   split_options: {size: 1, by: 2, with_index: 3}).enqueue(ChronusSftpFeed::Service::MigrationSplitter.new(), "run_updater", @modified_chunks, 0, klass, options, @config)
...
class DjSplit
  ...
  def enqueue(object, method_name, *args)
    ...
    indexing_index = get_indexing_index if @split_options[:with_index]
    sliced_ids_array = get_sliced_ids(args[splitting_index])

    sliced_ids_array.each_with_index do |slice_set, index|
      args[splitting_index] = slice_set
      args[indexing_index] = index if @split_options[:with_index]
  ...
  def get_indexing_index
    @split_options[:with_index] - 2
  end
